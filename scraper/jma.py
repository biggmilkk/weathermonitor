...
async def _fetch_office_json(client: httpx.AsyncClient, office: str,
                             allowed_code_to_name: Dict[str, str], sem: asyncio.Semaphore) -> List[dict]:
    """Fetch and parse a single office JSON."""
...
async def scrape_jma_async(conf: dict, client: httpx.AsyncClient) -> dict:
    """Async JMA scraper — fetch all offices concurrently via shared client."""
    try:
    ...
    sem = asyncio.Semaphore(12)  # limit JMA concurrency
